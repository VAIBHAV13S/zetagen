...
contract ZetaGenNFT is ERC721, ERC721URIStorage, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function mintAsset(
        ...
        emit AssetMinted(assetId, tokenId, to, tokenURI);
    ...
    function batchMintAssets(
        ...
        require(assetIds.length == tokenURIs.length, "Arrays length mismatch");
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            emit AssetMinted(assetIds[i], tokenId, recipients[i], tokenURIs[i]);
        }
    }
